...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === "register") {
        ...
        if (strlen($username) < 3) {
            ...
            if ($check->num_rows > 0) {
                $alert = "alert('⚠ Email already exists. Try logging in.');";
            } else {
    ...
    if ($action === "login") {
        ...
        if (empty($email) || empty($password)) {
            $alert = "alert('⚠ Email and password are required.'); window.location.href='p1.php';";
        } else {
            $stmt = $conn->prepare("SELECT user_id, password FROM user_master WHERE email = ?");
            if (!$stmt) {
                $alert = "alert('❌ SQL error: " . $conn->error . "'); window.location.href='p1.php';";
            } else {
                ...
                if ($stmt->num_rows === 1) {
                    ...
                    if (password_verify($password, $hashed)) {
                        ...
                        $alert = "alert('❌ Incorrect password.'); window.location.href='p1.php';";
                    }
                } else {
                    $alert = "alert('❌ Email not found.'); window.location.href='p1.php';";
                }
                $stmt->close();
            }
        }
...
<head>
  ...
  <style>
    /* Same styles as you gave, no changes here */
